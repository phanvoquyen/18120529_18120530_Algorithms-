...
public:
	...
	// hàm nén
	void Compress(const string& file, string filecompress)
	{
		char c;
		...
		//tach ten file va duoi.txt
		string a = filecompress;
		char*b = NULL;
		string newfilename = strtok_s((char*)a.c_str(), ".", &b);;
		newfilename += ".huffman";
	...
	// thực hiện tạo mã code theo quy tắc duyệt cây huffma, sang trái thêm '0', sang phải thêm '1'
	...
	void UnCompress(const string& CompressFile, string filedecompress)
	{
		...
		//tạo biến lưu dữ tên file cần lưu sau giải nén
		string a = filedecompress;
		char*b = NULL;
		//cắt phần .huff
		string newfile = strtok_s((char*)a.c_str(), ".", &b);
		//vì để không chùng tên với file ban đầu nên file sau giải nén sẽ có sô (1) đằng sau.
		newfile = newfile + "(1)" + suf;
		//tạo tên file lưu tên đã qua xử lí
		const string newfilename(newfile);
		//tạo file mới
		ofstream ofs(newfilename, ios::binary);
		CharInfo chInfo;
		// đọc dữ liệu, dựng lại bảng hashInfo
		while (1)
		{
			ifs.read((char*)&chInfo, sizeof(chInfo));
			//cout << "----------"<<(char*)&chInfo<<endl;
			if (chInfo.count > 0)
			{
				hashInfo[(unsigned char)chInfo.ch].count = chInfo.count;
				/*cout << "===="<< hashInfo[(unsigned char)chInfo.ch].ch << endl;
				cout << "***" << hashInfo[(unsigned char)chInfo.ch].count << endl;*/
			}
			else
			{
				break;
...
void TestCompress(const string &file, string filecompress)
{
	FileCompress f;
	f.Compress(file, filecompress);
}

void TestUnCompress(const string &file, string filedecompress)
{
	FileCompress f;
	f.UnCompress(file, filedecompress);
}