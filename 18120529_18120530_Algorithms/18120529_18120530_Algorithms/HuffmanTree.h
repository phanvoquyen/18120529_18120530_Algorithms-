...
struct HuffmanTreeNode
{
	...
	HuffmanTreeNode(const T &t = T()) :t(t), left(nullptr), right(nullptr), parent(nullptr)
	{};
};
...
public:
	...
	HuffmanTree(T *t, size_t size, const T & illegal) // sử dụng mảng dữ liệu T để xây dựng trọng số cây huffman
	{
		...
		while (minheap.size() > 1)
		{
			...
			minheap.pop();
			Node* parenttmp = new Node(lefttmp->t + righttmp->t);
			// tạo quan hệ cây
			parenttmp->left = lefttmp;
			parenttmp->right = righttmp;
